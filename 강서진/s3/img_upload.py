...
def file_upload():
    if request.method == 'POST':
        ...
        f.filename = newname
        imgpath = f"./static/{f.filename}"
        f.save(imgpath)

        ret = s3_put_object(s3, AWS_S3_BUCKET_NAME, imgpath, f.filename)

        if ret:
            print("success")

            img_url = f"https://{AWS_S3_BUCKET_NAME}.s3.{AWS_S3_BUCKET_REGION}.amazonaws.com/{f.filename}"
